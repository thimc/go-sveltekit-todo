...
func (h *TodoHandler) HandleGetTodoByID(c *fiber.Ctx) error {
	...
	if todo == nil {
		return types.NewApiResponse(false, "no such record", http.StatusNotFound)
	}
...
func (h *TodoHandler) HandleUpdateTodo(c *fiber.Ctx) error {
	id, err := strconv.Atoi(c.Params("id"))
	if err != nil {
		return types.NewApiResponse(false, err.Error(), http.StatusBadRequest)
	}
...
func (h *TodoHandler) HandlePatchTodo(c *fiber.Ctx) error {
	...
	return types.NewApiResponse(false, "not implemented", http.StatusNotImplemented)
